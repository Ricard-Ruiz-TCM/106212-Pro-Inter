...
function flee()
    ...
    if (target - position):Magnitude() < flee_distance then
        desired_velocity = (target - position):Normalized() * max_velocity
...
function update()
    ...
    local flee_velocity = flee()
    local hunter = Vector2.FromTable(agent:GetTarget())
    local target = Vector2.New(1000, 1000)
    ...
    if (hunter - position):Magnitude() < flee_distance then
        velocity = flee_velocity
    elseif (target - position):Magnitude() > stop_distance then
